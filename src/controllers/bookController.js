...
const getParticularBooks = async function(req, res) {
    ...
    let allBooks = await BookModel.find({
        $or: [{ authorName: { $eq: nm } }, { year: { $eq: year } }, { bookName: { $eq: name } }, { totalPages: { $eq: totalpage } }]
    }).select()
    res.send({ msg: allBooks })
}
...
const getRandomBooks = async function(req, res) {
    let allBooks = await BookModel.find({ stockAvailable: true, totalPages: { $gte: 500 } })
    res.send({ msg: allBooks })
}
